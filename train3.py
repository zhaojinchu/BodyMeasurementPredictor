...
# Skeletal features to be constrained
...
# **Force Skeletal Features to Zero Change**
for i, feature in enumerate(output_features):
    if feature in skeletal_features:
        y_scaled[:, i] = 0  # Set percentage change to zero for skeletal features
...
# **Custom Weighted Loss Function to Reduce Skeletal Feature Contribution**
class WeightedLoss(nn.Module):
    def __init__(self, skeletal_features, output_features, weight_factor=0.1):
        super(WeightedLoss, self).__init__()
        self.skeletal_indices = [output_features.index(f) for f in skeletal_features]
        self.weight_factor = weight_factor  # Scale down skeletal contribution

    def forward(self, outputs, targets):
        loss = F.smooth_l1_loss(outputs, targets, reduction="none")
        for i in self.skeletal_indices:
            loss[:, i] *= self.weight_factor  # Reduce skeletal feature impact on loss
        return loss.mean()

# Apply Weighted Loss
criterion = WeightedLoss(skeletal_features, output_features, weight_factor=0.1)
